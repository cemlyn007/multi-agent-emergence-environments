...
def _std_from_mean_and_square(mean, square):
    var_est = tf.compat.v1.to_float(square) - tf.square(mean)
...
class EMAMeanStd(object):
    ...
    def update_op(self, x, axes=(0,)):
        ...
        old_weight_fp32 = tf.compat.v1.to_float(old_weight)
        scaled_weight_fp32 = tf.compat.v1.to_float(scaled_weight)
        return tf.group(
            ...
            tf.assign(self._debiasing_term, tf.compat.v1.to_float(_interpolate(old=tf.cast(self._debiasing_term, tf.float64), new=one, old_weight=old_weight, scaled_weight=scaled_weight))),
            # do an interpolation on the expected value of X
            tf.assign(self._biased_mean, _interpolate(old=self._biased_mean, new=tf.reduce_mean(tf.compat.v1.to_float(x), axis=axes), old_weight=old_weight_fp32, scaled_weight=scaled_weight_fp32)),
            # do an interpolation on the expected value of X^2
            tf.assign(self._biased_sq, _interpolate(old=self._biased_sq, new=tf.reduce_mean(tf.square(tf.compat.v1.to_float(x)), axis=axes), old_weight=old_weight_fp32, scaled_weight=scaled_weight_fp32)),
        )
