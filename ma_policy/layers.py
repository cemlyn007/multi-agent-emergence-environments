...
def entity_concat(inps):
    ...
    '''
    with tf.compat.v1.variable_scope('concat_entities'):
...
def concat_entity_masks(inps, masks):
    ...
    with tf.compat.v1.variable_scope('concat_masks'):
        shapes = [shape_list(_x) for _x in inps]
        new_masks = []
...
def residual_sa_block(inp, mask, heads, n_embd,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        post_a_mlp = tf.compat.v1.layers.dense(a,
                                     n_embd,
        ...
        if n_mlp > 1:
            ...
            mlp = tf.compat.v1.layers.dense(mlp,
                                  n_embd,
...
def self_attention(inp, mask, heads, n_embd, layer_norm=False, qk_w=1.0, v_w=0.01,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        entity_mask = None
        if mask is not None:
            with tf.compat.v1.variable_scope('expand_mask'):
        ...
        with tf.compat.v1.variable_scope('flatten_heads'):
...
def stable_masked_softmax(logits, mask):
    ...
    '''
    with tf.compat.v1.variable_scope('stable_softmax'):
...
def qkv_embed(inp, heads, n_embd, layer_norm=False, qk_w=1.0, v_w=0.01, reuse=False):
    '''
        ...
        Args:
            ...
            reuse (bool) -- tf reuse
    '''
    with tf.compat.v1.variable_scope('qkv_embed'):
        bs, T, NE, features = shape_list(inp)
        if layer_norm:
        ...
        value = tf.compat.v1.layers.dense(inp,
                                n_embd,
...
def layernorm(x, scope, epsilon=1e-5, reuse=False):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        gain = tf.compat.v1.get_variable('gain', [n_state], initializer=tf.constant_initializer(1))
        bias = tf.compat.v1.get_variable('bias', [n_state], initializer=tf.constant_initializer(0))
