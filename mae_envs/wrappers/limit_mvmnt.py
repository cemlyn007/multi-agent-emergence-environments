...
class RestrictAgentsRect(gym.RewardWrapper):
    ...
    def reset(self):
        ...
        if self.penalize_objects_out:
            obj_body_idxs = ([sim.body_name2id[f'moveable_box{i}'] for i in np.where(self.metadata['curr_n_boxes'])[0]] +
                             [sim.body_name2id[f'ramp{i}:ramp'] for i in np.where(self.metadata['curr_n_ramps'])[0]])
            self.obj_body_idxs = np.array(obj_body_idxs)
        return obs

    def reward(self, reward):
        sim = self.unwrapped.sim
        agent_pos = sim.data.xpos[self.agent_body_idxs, :2]
        outside_rect = np.any(np.abs(agent_pos - self.rect_middle) > (self.rect_size / 2), axis=1)
        if self.penalize_objects_out:
            obj_pos = sim.data.xpos[self.obj_body_idxs, :2]
